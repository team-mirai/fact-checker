 ...
 */
export async function notifySlack(factCheckResult: string, originalTweet: string, tweetUrl?: string) {
  const isOk = /^OK/i.test(factCheckResult);
  const truncatedTweet = originalTweet.length > 180 ? `${originalTweet.slice(0, 180)}…` : originalTweet;
  const detectionTime = new Date().toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' });
  ...
  let citationText = "";
  const detailsMatch = factCheckResult.match(/<details>[\s\S]*?<summary>([\s\S]*?)<\/summary>([\s\S]*?)<\/details>/i);
  ...
  const blocks: Block[] = [
    {
      ...
      text: {
        type: "plain_text",
        text: isOk ? "✅ ファクトチェック完了（問題なし）" : "🔍 ファクトチェック要請",
      }
    },
    // ステータスインジケーター（緑/赤の円と検証結果）
    {
      ...
      elements: [
        {
          type: "mrkdwn",
          text: isOk ? ":large_green_circle: *検証結果: OK*" : ":red_circle: *検証結果: 要確認*"
        }
      ]
    },
    {
  ...
  await slack.chat.postMessage({
    ...
    text: isOk ? "✅ ファクトチェック完了（問題なし）" : "🔍 ファクトチェック要請 [要確認]",
    blocks: blocks,
  });
}
...
slackApp.action('approve_and_post', async ({ ack, body, client }) => {
  ...
  try {
    ...
    const status = [
      '✅ ファクトチェック結果',
      '',
      originalTweet.length > 200 ? originalTweet.slice(0, 200) + '…' : originalTweet,
      '',
      '—– 誤りの指摘 —–',
    ...
    await client.chat.update({
      ...
      blocks: [
        {
          type: 'section',
          text: { type: 'mrkdwn', text: ':white_check_mark: X への投稿が完了しました。' },
        },
        {
          type: 'section',
      ...
      if (channel && ts) {
        ...
        await client.chat.update({
          ...
          blocks: [
            {
              type: 'section',
              text: { type: 'mrkdwn', text: ':x: エラーが発生しました。管理者に連絡してください。' },
            },
          ],
        });
      }
  ...
  try {
    ...
    await client.chat.update({
      ...
      blocks: [
        {
          type: 'section',
          text: { type: 'mrkdwn', text: ':pencil2: 編集機能は現在開発中です。次期アップデートをお待ちください。' },
        },
      ],
    });
  } catch (error) {
  ...
  try {
    ...
    await client.chat.update({
      ...
      blocks: [
        {
          type: 'section',
          text: { type: 'mrkdwn', text: ':x: このファクトチェック要請は却下されました。' },
        },
      ],
    });
  } catch (error) {
